...
void Iinit_ESP(char *envoi){
  ...
  envoyer_UART(envoi);//désactiver echoAT+SLEEP=2
  __delay_ms(300);
  sprintf(envoi,"AT+SLEEP=2\r\n");//On autorise plusieurs connexions parallèles
  envoyer_UART(envoi);
  __delay_ms(300);
...
void Init_Interruption(void){
      ...
      PIE1bits.TMR1IE = 1;    // Active l'interruption Timer1 p.112
      ...
      IPR1bits.TMR1IP = 0;   // Timer1 en basse priorité p.121
...
void Init_Timer1(void)// // Configuration du Timer0 pour 0,2Hz (T=5s)
{
 T1CONbits.TMR1ON=0; //timer désactivé p.166
 T1CONbits.TMR1CS = 0;   // Sélection de Crystal oscillator on SOSCI/SOSCO pins (~31 kHz) p.166
 T1CONbits.T1CKPS = 2;   // Prescaler 1:4 (Fréquence 7750Hz soit T=129us) p.166
 T1CONbits.T1SOSCEN = 1;  // Active l'oscillateur secondaire du Timer1 p.166
  T1CONbits.T1SYNC = 1;  // ne pas synchroniser sur un oscillateur externe p.166
 T1CONbits.T1RD16=1; //configuration du compteur sur 16bits p.166
 // comptage : 5/130us = 38750
 // Calcul du préchargement : 65536 - (38750) = 26786
 // En hexadécimal : 26786 = 0x68A2
 TMR1H= 0x68; // Partie haute du registre TMR1 (0x68)
 TMR1L = 0xA2; // Partie basse du registre TMR1 (0xA2)
 PIR1bits.TMR1IF = 0; // Reset du flag d'interruption
}
//-----------------------------------------------------------------------------------------------------------------
...
void __interrupt (high_priority) HighISR() {
    ...
    char caractere_recu;
    unsigned short etat_relai_1;
    unsigned short etat_relai_2;
    
    if (PIR1bits.RCIF == 1) {
        //resynchronisation de l uart (pete du pemier octet)
        RCSTAbits.CREN = 0;  // désactive le module réception
        __delay_ms(1);
        raz=RCREG1;
        RCSTAbits.CREN = 1;  // active le module réception
        do {
            caractere_recu = UartReadChar();
            data[i] = caractere_recu;
            i++;
        } while(((caractere_recu != '\n' )||i<=2)&&(i <= 11));  // Lire jusqu'à '\n' ou taille max ou >(message)
        i=0;
        if(strncmp(data+2,"IPD,0,2:", 8) == 0){
            etat_relai_1=(data[11]-'0');
            etat_relai_2=(data[12]-'0');
            if(etat_relai_1){
                R1_ouvert();   
            }
            else{
        ...
        else if(strncmp(data+1,"CONNECT", 7) == 0){
...
//            INTCONbits.TMR0IF = 0;   // RAZ du flag TMR0IF
            T1CONbits.TMR1ON=1; //timer désactivé p.166
            TMR1H= 0x68; // Partie haute du registre TMR1 (0x68)
            TMR1L = 0xA2; // Partie basse du registre TMR1 (0xA2)
            PIR1bits.TMR1IF = 0; // Reset du flag d'interruption
        }
        else if(strncmp(data+1,"CLOSED", 6) == 0){
            //T0CONbits.TMR0ON=0; //timer déactivé p.154
            T1CONbits.TMR1ON=0; //timer désactivé p.166
            effacer(data);
        }
    }
...
//        INTCONbits.TMR0IF = 0;   // RAZ du flag TMR0IF
        ...
        humidite=SHT21_lecture_humidite(); //test mesure humidité 
        temperature=SHT21_lecture_Temperature(); // test mesure température
        sprintf(capteur,"%.1f*%.1f",temperature,humidite);
        sprintf(envoi,"AT+CIPSEND=0,9\r\n");
        RCSTAbits.CREN = 0;  // Désactive le module réception
        envoyer_UART(envoi);
        __delay_ms(100);
        envoyer_UART(capteur);
        RCSTAbits.CREN = 1;  // active le module réception
    }
}
