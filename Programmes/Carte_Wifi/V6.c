...
void __interrupt (high_priority) HighISR() {
    ...
    if (PIR1bits.RCIF == 1) { // Vérifie si une interruption de réception UART est déclenchée
        // Resynchronisation de l'UART (perte du premier octet du au temps de reveille de l'UART)
        ...
        if(strncmp(data+2,"IPD,0,2:", 8) == 0){ // Vérifie si la commande reçue est une commande de mise à jour des relais
            etat_relai_1 = (data[11] - '0'); // Conversion du caractère en valeur numérique (0 ou 1)
...
void __interrupt (low_priority) LowISR() {
    ...
    if (INTCONbits.TMR0IF){
        ...
        PIE1bits.RC1IE=0; //désactive les interruptions en réception (sur RX)) p.112
        RCSTAbits.CREN = 0;  // Désactive le module réception
        ...
        raz = RCREG1; // Vider le buffer de réception UART
        raz = RCREG1; // (Double lecture pour s?assurer que tout est vidé)
        PIR1bits.RCIF = 0; // Réinitialisation du flag de réception UART
        RCSTAbits.CREN = 1;  // active le module réception
        PIE1bits.RC1IE=1; //active les interruptions en réception (sur RX)) p.112
    }
}
